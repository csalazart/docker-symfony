...
if [ $VM_EXISTS_CODE -eq 1 ]; then
  ...
  "${DOCKER_MACHINE}" create -d virtualbox $PROXY_ENV "${VM_NAME}"
...
cat > /tmp/docker-bash-rc <<EOT
# Source global definitions
if [ -f $HOME/.bashrc ]; then
  . $HOME/.bashrc
fi
alias docker-compose='"${DOCKER_COMPOSE}" -p "$PROJECT_NAME" -f "$DOCKER_DIR/docker-compose.yml"'
cd "$DOCKER_DIR"
EOT
/bin/bash --rcfile "/tmp/docker-bash-rc"